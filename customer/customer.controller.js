...
exports.createCustomer = catchAsync(async (req, res, next) => {
  ...
  if (!regionId) {
    return next(new AppError("Region is required", 400));
  }

  if (!mongoose.Types.ObjectId.isValid(regionId)) {
    return next(new AppError("Invalid region ID format", 400));
  }
  ...
  const duplicated = await Customer.findOne({ name, region: regionId });

  if (duplicated) {
    if (duplicated.isActive === false) {
      return next(
        new AppError("Customer exists but is inActive. Reactivate instead of creating.", 400)
      );
    }

    return next(
      new AppError("Customer already exists in this region.", 400)
    );
  }

  const customerId = `CUS-${region.code}-${new Date()
    .toISOString()
    .slice(0, 10)
  ...
  const populatedCustomer = await customer.populate("region", "name code");
